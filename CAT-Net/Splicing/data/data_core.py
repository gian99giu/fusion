...
class SplicingDataset(Dataset):
    def __init__(self, crop_size, grid_crop, blocks=('RGB',), mode="train", DCT_channels=3, read_from_jpeg=False, class_weight=None,filename=''):
        self.dataset_list = []
        if mode == "train":
            ...
            self.dataset_list.append(arbitrary(crop_size, grid_crop, blocks, DCT_channels, filename, read_from_jpeg=read_from_jpeg))
        else:
